...
class ClientHandle(object):
    ...
    def forever_run(self):
        ...
        while not exit_flag:                                                                                           #死循环向服务器获取监控配置
              if time.time() - config_last_update_time > settings.configs['ConfigUpdateInterval']:                      #300秒获取一次
              ...
              for service_name,val in self.monitored_services['services'].items():
                  if len(val) == 2:# means it's the first time to monitor第一次是添加，之后是耿新
                      self.monitored_services['services'][service_name].append(0)
                  monitor_interval = val[1]
                  last_invoke_time = val[2]
                  if time.time() - last_invoke_time > monitor_interval: #needs to run the plugin  当前时间减去上次监控时间，如果大于间隔时间就运行监控插件
