...
class ClientHandler(object):
    ...
    def fetch_configs(self):
        try:
            ...
            template_list= host_obj.templates.select_related()                  #去除主机关联的模板

            for host_group in host_obj.host_groups.select_related():
                template_list.extend( host_group.templates.select_related() )   #添加所有关联主机组的模板
            print(template_list)
            for template in template_list:
                ...
                for service in template.services.select_related(): #loop each service
                    print(service)
                    self.client_configs['services'][service.name] = [service.plugin_name,service.interval]   #   列出所有监控服务、插件、监控间隔
